...
export function DealerConsumablesLandingPage() {
      ...
      const entries = await Promise.all(
        PARTS_AND_TONER_BRANDS.map(async (brand) => {
          try {
            ...
            const response = await fetch(`/api/dealer/product?${params.toString()}`, {
              cache: "no-store",
            });
            const payload = await safeReadJsonResponse<{ data?: Product[] }>(
              response,
              "DealerConsumablesLandingPage previews",
            );
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-x-1.25 gap-y-10 sm:grid-cols-2">
          {filteredBrands.map((brand) => (
            <CategoryCard
              key={brand.slug}
              title={brand.label}
              href={`/dealer/products/consumables/${brand.slug}`}
...
export function DealerConsumablesBrandPage({ brandSlug }: { brandSlug: string }) {
      ...
      const entries = await Promise.all(
        PARTS_AND_TONER_TYPES.map(async (type) => {
          try {
            ...
            const payload = await safeReadJsonResponse<{ data?: Product[] }>(
              response,
              "DealerConsumablesBrandPage previews",
            );
    ...
    <div className="min-h-screen text-black font-myriad">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:py-10">
        ...
        <nav className="mb-8 flex flex-wrap items-center gap-2 text-sm text-black/55">
          ...
          <span>/</span>
          <Link href="/dealer/products/consumables" className="hover:text-[var(--brand-cyan)]">Parts and Toners</Link>
          <span>/</span>
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-x-1.25 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
          {filteredTypes.map((type) => (
            <CategoryCard
              ...
              href={`/dealer/products/consumables/${brand.slug}/${type.slug}`}
...
export function DealerConsumablesProductsPage({
  brandSlug,
  typeSlug,
}: {
  brandSlug: string;
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const payload = await safeReadJsonResponse<{ data?: Product[]; error?: string }>(
        response,
        "DealerConsumablesProductsPage products",
      );
    ...
    <div className="min-h-screen text-black font-myriad">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:py-10">
        ...
        <nav className="mb-8 flex flex-wrap items-center gap-2 text-sm text-black/55">
          ...
          <Link href="/dealer/products/consumables" className="hover:text-[var(--brand-cyan)]">Parts and Toners</Link>
          <span>/</span>
          <Link href={`/dealer/products/consumables/${brand.slug}`} className="hover:text-[var(--brand-cyan)]">{brand.label}</Link>
          <span>/</span>
...
export function DealerConsumableSlugResolver({
  brandSlug,
  typeSlug,
  productSlug,
  render,
  ...
  useEffect(() => {
    ...
    async function resolveProduct() {
      ...
      try {
        ...
        const payload = await safeReadJsonResponse<{ data?: Product[]; error?: string }>(
          response,
          "DealerConsumableSlugResolver product",
        );
