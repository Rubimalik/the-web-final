...
import { listCustomerOrders, type OrderRecord } from "@/lib/orders-store";
...
export default async function OrdersPage() {
  ...
  if (!auth?.user) {
    redirect("/signin?from=%2Forders");
  }

  const { data: orders } = await listCustomerOrders(auth.user.id, {
    page: 1,
    limit: 20,
  });
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-black/60 text-sm sm:text-base">
              Track order requests placed through the BuySupply store.
            </p>
          </div>
          <Link
            href="/products"
